...
<Callout type="info">
`@billsdk/stripe` is optional if you only need free plans. `@billsdk/drizzle-adapter` can be swapped for other database adapters.
</Callout>
</Step>
...
Get these from [Stripe Dashboard → API Keys](https://dashboard.stripe.com/apikeys) and [Webhooks](https://dashboard.stripe.com/webhooks).
