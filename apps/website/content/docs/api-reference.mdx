...
Utility functions for checking trial status. Import from `billsdk`:

```typescript
import { isInTrial, daysRemainingInTrial, getTrialInfo } from "billsdk";
```
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `subscription` | `Subscription` | Yes | The subscription to check |
| `now` | `Date` | No | Current time (default: `new Date()`) |

**Returns:** `number` (0 if not trialing)

### getTrialInfo

Get full trial information for a subscription.

```typescript
const trial = getTrialInfo(subscription);
if (trial?.isTrialing) {
  console.log(`${trial.daysRemaining} days left`);
  console.log(`Trial ends: ${trial.trialEnd}`);
}
```
...
**Returns:** `{ isTrialing: boolean; daysRemaining: number; trialEnd: Date | null } | null`

Returns `null` if the subscription has never had a trial.
